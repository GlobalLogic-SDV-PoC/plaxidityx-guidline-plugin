...
def download_file(url, output_path):
    ...
    try:
        ...
        fatal_error(f"Download failed: {str(e)}")
    except IOError as e:
        fatal_error(f"File I/O error during download: {str(e)}")
...
def ensure_package_directory_exists():
    ...
    try:
        ...
        fatal_error(f"Failed to create package directory: {str(e)}")
    ...
    try:
        ...
        if os_name == 'windows':
            if machine in ['amd64', 'x86_64']:
                ...
                fatal_error(f"Unsupported Windows architecture: {machine}")
        elif os_name == 'linux':
            if machine in ['amd64', 'x86_64']:
                filename = "codeql-linux64.zip"
            else:
                fatal_error(f"Unsupported Linux architecture: {machine}")
        elif os_name == 'darwin':
            if machine in ['arm64', 'aarch64']:
                ...
                fatal_error(f"Unsupported macOS architecture: {machine}")
        else:
            fatal_error(f"Unsupported operating system: {os_name}")
        ...
        fatal_error(f"Failed to install CodeQL CLI: {str(e)}")
...
def check_os_and_codeql():
    ...
    try:
        ...
        try:
            ...
            fatal_error(f"Error checking CodeQL version: {e.output}")
    except Exception as e:
        fatal_error(f"Failed during OS and CodeQL verification: {str(e)}")
...
def run_codeql_analysis(c_path, script_path):
    try:
        ...
        if not os.path.exists(c_path):
            fatal_error(f"C source path does not exist: {c_path}")
        ...
        fatal_error(f"Error executing CodeQL command: {e}")
...
def make_executables_executable(directory_path, recursive=True):
    ...
    try:
        ...
        fatal_error(f"Failed to set executable permissions: {str(e)}")
...
def main():
    try:
        ...
        if len(sys.argv) < 10:
            fatal_error("Usage: python wrap.py <code_path> <build_script> <repo_name> <branch_name> <commit_hash> <auth_url> <app_url> <username> <password> [user_archive]")
